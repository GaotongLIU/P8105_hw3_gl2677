...
* `packaged vegetables fruits` is the most frequently ordered aisles, and `dog food care` is the least frequently ordered ones.
...
states_2010 = overall_health %>% 
  ...
  arrange(desc(n)) 
```
* In 2002, total ``r nrow(states_2002)`` states ([most locations first]``r pull(states_2002, state)``) were observed at 7 or more locations. The range of the number of locations in the state is from `r max(pull(states_2002, n))` to `r min(pull(states_2002, n))`.
* In 2010, total ``r nrow(states_2010)`` states ([most locations first]``r pull(states_2010, state)``) were observed at 7 or more locations. The range of the number of locations in the state is from `r max(pull(states_2010, n))` to `r min(pull(states_2010, n))`.
* The number of locations in the state increases in 2010 comparing to 2002. 
...
* The peaks of `data_value` decrease for `Poor` and `Fair` in 2010 comparing to thoes in 2006, and the peaks of `data_value` increase for Excellent`, `Good` and `Very good` in 2010 comparing to thoes in 2006.
