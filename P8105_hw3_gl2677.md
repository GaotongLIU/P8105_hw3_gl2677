...
```
  ...
  - There are 50400 observations and 6 variables in `acticity` dataset.
    The variables are week, day\_id, day, minute, activity\_counts,
    weekday. For each observation, the dataset shows the activity counts
    for the exact minute of a 24-hour day starting at midnight on exact
    day and exact week.
